...
Нужно граматно работать с метриками, например закинув идентификатор запроса их станет тысячи, что приведет к смерти системы.
Тк такие системы приспособлены под хранения, небольшого числа метрик.
Все это деньги на жедезо и прочее.

Дежурные на резкое увеличение метрик
Автоматизация следящая за этим.
...
Что такое пейдж?
Пейдж - толстый алерт - его надо принять и он сохранит время принятия
...
TBSLO - давайте каждый момент времени назовём хорошим, если он удовл SLO, иначе плохим и будем считать процент хорошего времени
RBSLO - давайте будем считать процент запросов удовл SLO
Минусы TBSLO - всё время весит одинаково, статистика портится, если пользователей мало
Минусы RBSLO - если считать процент ошибочных (а не успешных), то репиты попортят всю статистику (хотя это +- норм поведение) + если мы каким-то образом нафармили волну успешных запросов, то сбой после них не будет отображён
